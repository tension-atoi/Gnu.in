...
|---|------|-------|-------------|--------|
| 1 | R1: GitHub API Integration | Add PAT input field to settings, rewrite `GitHubClient` to query GitHub REST API using `requests` and token, parse git remote URL for owner/repo, create `tests/test_github_api.py`. | None | DONE |
| 2 | R2: UI Styling Adaptation | Extract styling parameters from `SysterTheme.hpp` and apply them to views (`main_window.py`, `github_panel.py`, etc.) using PySide6 stylesheet syntax. | None | DONE |
| 3 | R3: Local Install Script | Create `install.sh` at project root to deploy the application, configure dependencies using user-level python tools, and create standard `.desktop` file. | None | DONE |
| 4 | Phase 1: E2E Test Compatibility | Poll for `TEST_READY.md` and verify that the application passes all E2E tests (Tiers 1-4). | M1, M2, M3 | DONE |
| 5 | Phase 2: Adversarial Hardening | Run adversarial testing (Tier 5) to verify code correctness, safety, and robustness. | M4 | DONE |
